...
void lua_do_all (void) {
  ...
  while (p < pos) {
	...
	case 3:
      ...
      break;
	case 4:
      do_send_photo (CODE_input_media_uploaded_video,((peer_t *)lua_ptr[p])->id, lua_ptr[p + 1]);
      //tfree_str (lua_ptr[p + 1]);
      p += 2;
      break;
	case 5:
      do_send_photo (CODE_input_media_uploaded_audio,((peer_t *)lua_ptr[p])->id, lua_ptr[p + 1]);
      //tfree_str (lua_ptr[p + 1]);
      p += 2;
      break;
	case 6:
      do_send_photo (CODE_input_media_uploaded_document,((peer_t *)lua_ptr[p])->id, lua_ptr[p + 1]);
      //tfree_str (lua_ptr[p + 1]);
      p += 2;
      break;
    default:
...
static int send_video_from_lua (lua_State *L) {
  if (MAX_LUA_COMMANDS - pos < 4) {
    lua_pushboolean (L, 0);
    return 1;
  }
...
static int send_audio_from_lua (lua_State *L) {
  if (MAX_LUA_COMMANDS - pos < 4) {
    lua_pushboolean (L, 0);
    return 1;
  }
...
static int send_document_from_lua (lua_State *L) {
  if (MAX_LUA_COMMANDS - pos < 4) {
    lua_pushboolean (L, 0);
    return 1;
  }
